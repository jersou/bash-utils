...
main() {
  ...
  sed ${sed_opt} \
    -e 's/utils:blue|utils:cyan|utils:debug|utils:error|utils:green|utils:log|utils:orange|utils:purple|utils:red|utils:warn|utils:white/echo/g' \
    ...
    -e 's/utils:run_main *("\$@")?/set -o errexit\nset -o nounset\nset -o pipefail\nset -o errtrace\nshopt -s inherit_errexit\n[[ ${TRACE:-false} != true ]] || set -o xtrace\nmain/g' \
